...
const ApplayCouponHook = (cartItems) => {
    ...
    const handelSubmitCoupon = async () => {
        if (couponName === "") {
            notify("Veuillez saisir le coupon", "warn")
            return
        }
        setLoading(true)
    ...
    useEffect(() => {
        ...
        if (loading === false) {
            ...
            if (res && res.status === 200) {
                notify("Le coupon a été appliqué avec succès", "success")
                setTimeout(() => {
                    window.location.reload(false)
                }, 1000);

            } else {
                notify("Ce coupon est incorrect ou extrait", "warn")
                setTimeout(() => {
                    window.location.reload(false)
                }, 1000);
            }
    ...
    const handelCheckout = () => {
        if (cartItems.length >= 1) {
            ...
            notify("Veuillez d'abord ajouter des produits au panier", "warn")
        }
    }
