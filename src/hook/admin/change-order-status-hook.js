...
const ChangeOrderStatusHook = (id) => {
    ...
    const changePayOrder = async () => {
        ...
        if (pay === 'true') {
            ...
            console.log('Veuillez choisir une valeur')
        }
    ...
    useEffect(() => {
        if (loading === false) {
            if (resOneOrder && resOneOrder.status === 200) {
                notify("Le statut de paiement a été modifié avec succès", "success")
                setTimeout(() => {
                    window.location.reload(false)
                }, 1000);
            } else {
                notify("Il y a un problème avec le processus de changement", "error")
            }
    ...
    useEffect(() => {
        if (loadingDeliver === false) {
            if (resDeliverOrder && resDeliverOrder.status === 200) {
                notify("Le statut de connexion a été modifié avec succès", "success")
                setTimeout(() => {
                    window.location.reload(false)
                }, 1000);
            } else {
