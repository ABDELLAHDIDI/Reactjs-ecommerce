...
const AdminAddProductsHook = () => {
    ...
    const [priceBefore, setPriceBefore] = useState('Le prix avant la remise');
    const [priceAftr, setPriceAftr] = useState('Le prix après remise');
    const [qty, setQty] = useState('La quantité disponible');
    ...
    useEffect(() => {
        ...
        if (loading === false) {
            ...
            setProdName('')
            setProdDescription('')
            setPriceBefore('Le prix avant la remise')
            setPriceAftr('Le prix après remise ')
            setQty('La quantité disponible')
            SetBrandID(0)
            setSeletedSubID([])
            ...
            if (product) {
                if (product.status === 201) {
                    notify("Il a été ajouté réussi", "success")
                } else {
                    notify("Il existe un problème", "error")
                }
            }
        }
    }, [loading])
