...
const ProfileHook = () => {
    ...
    useEffect(() => {
        if (loading === false) {
            ...
            if (res && res.status === 200) {
                notify("Parler avec succès", "success")
                ...
                notify("La mise à jour a échoué", "warn")
            }
        }
    }, [loading])
    ...
    const changePassword = async () => {
        ...
        if (confirmNewPassword != newPassword) {
            notify("La confirmation du mot de passe n'est pas identique", "warn")
            return
        }
        setLoadingPass(true)
    ...
    useEffect(() => {
        if (loadingPass === false) {
            ...
            if (resPass && resPass.status === 200) {
                notify("Le mot de passe a été modifié avec succès", "success")
                setTimeout(() => {
