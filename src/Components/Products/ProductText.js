    ...
    <div>
      ...
      <Row>
        <Col md="8" className="mt-1 d-flex">
          ...
          <div className="cat-text d-inline">La quantité disponible : {item.quantity} </div>
        ...
        <Col md="12">
          {item.priceAfterDiscount >= 1 ? (
            <div className="product-price d-inline px-3 py-3 border">
              <span style={{ textDecorationLine: 'line-through' }}> {item.price}</span> {item.priceAfterDiscount} DH
            </div>) : <div className="product-price d-inline px-3 py-3 border"><span> {item.price}</span> DH </div>
          }
          <div onClick={addToCartHandel} className="product-cart-add px-3 py-3 d-inline mx-3">Ajouter au chariot</div>
        </Col>
      </Row>
      <ToastContainer />
    </div>
